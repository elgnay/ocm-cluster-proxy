...
type RBACPermissionBuilder interface {
	...
	// BindClusterRoleToRegistrationSubject ensures a cluster role and binds to subjects from addon.Status.Registrations.
	...
	BindClusterRoleToRegistrationSubject(clusterRole *rbacv1.ClusterRole) RBACPermissionBuilder
	// BindRoleToRegistrationSubject ensures a role and binds to subjects from addon.Status.Registrations.
	// It looks for the registration with signerName == KubeAPIServerClientSignerName.
	// Both subject.User and subject.Groups will be included in the binding.
	// This is useful for token-based authentication where subjects are dynamically set by the addon agent.
	BindRoleToRegistrationSubject(role *rbacv1.Role) RBACPermissionBuilder
...
func (p *permissionBuilder) BindClusterRoleToRegistrationSubject(clusterRole *rbacv1.ClusterRole) RBACPermissionBuilder {
...
func (p *permissionBuilder) BindRoleToRegistrationSubject(role *rbacv1.Role) RBACPermissionBuilder {
